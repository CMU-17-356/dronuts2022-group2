...
test('Can append to an empty cart', () => {
  const cart: UserCart = { 'donut_orders': [] };
  const cart2 = cartReducer(cart, { 'name': 'set', 'donut': donut1, 'quantity': 10 });
...
test('Can update a non empty cart', () => {
  const cart: UserCart = { 'donut_orders': [{ 'donut': donut1, 'quantity': 5 }, { 'donut': donut2, 'quantity': 3 }] };
  const cart2 = cartReducer(cart, { 'name': 'set', 'donut': donut1, 'quantity': 1 });
  expect(cart2).toStrictEqual({
...
test('Can delete from a non empty cart', () => {
  const cart: UserCart = { 'donut_orders': [{ 'donut': donut1, 'quantity': 5 }, { 'donut': donut2, 'quantity': 3 }] };
  const cart2 = cartReducer(cart, { 'name': 'set', 'donut': donut1, 'quantity': 0 });
  expect(cart2).toStrictEqual({
...
test('Can clear a non empty cart', () => {
  const cart: UserCart = { 'donut_orders': [{ 'donut': donut1, 'quantity': 5 }, { 'donut': donut2, 'quantity': 3 }] };
  const cart2 = cartReducer(cart, { 'name': 'clear' });
  expect(cart2).toStrictEqual({
    'donut_orders': [],
  });
  expect(cart).not.toBe(cart2);
