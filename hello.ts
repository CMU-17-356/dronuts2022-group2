...
import { DroneModel } from './frontend/database/schemas/drone_schema';
...
/** Add a new order to the Drones collection in the DB.
 *
 * @param req.body is an drone object.
 */
app.post('/new_drone', (req: express.Request, res: express.Response) => {
    console.log(req.body);
    ...
    drone
        ...
        .then((result: any) => {
            console.log(result);
            res.status(201).json({
                message: 'Handling POST requests to /new_drone',
                addedDrone: drone,
            });
        })
        .catch((err: any) => {
